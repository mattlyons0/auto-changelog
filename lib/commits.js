...
const BODY_FORMAT = '%B';
const FALLBACK_BODY_FORMAT = '%s%n%n%b'; // https://help.github.com/articles/closing-issues-via-commit-messages

const DEFAULT_FIX_PATTERN = /(?:close[sd]?|fixe?[sd]?|resolve[sd]?)\s(?:#(\d+)|(https?:\/\/.+?\/(?:issues|pull|pull-requests|merge_requests)\/(\d+)))/gi;
const MERGE_PATTERNS = [/Merge pull request #(\d+) from .+\n\n(.+)/, // Regular GitHub merge
/^(.+) \(#(\d+)\)(?:$|\n\n)/, // Github squash merge
/Merged in .+ \(pull request #(\d+)\)\n\n(.+)/, // BitBucket merge
/Merge branch .+ into .+\n\n(.+)[\S\s]+See merge request [^!]*!(\d+)/ // GitLab merge
];

async function fetchCommits(remote, options, branch = null, onProgress) {
  const command = branch ? `git log --merges ${branch}` : 'git log --merges';
  const format = await getLogFormat();
  console.log(`${command} --shortstat --pretty=format:${format}`);
  const log = await (0, _utils.cmd)(`${command} --shortstat --pretty=format:${format}`, onProgress);
  console.log('done');
  return await parseCommits(log, remote, options);
}

async function getLogFormat() {
  const gitVersion = await (0, _utils.getGitVersion)();
  const bodyFormat = gitVersion && _semver.default.gte(gitVersion, '1.7.2') ? BODY_FORMAT : FALLBACK_BODY_FORMAT;
...
async function parseCommits(string, remote, options = {}) {
  let commits = string.split(COMMIT_SEPARATOR).slice(1).map(commit => parseMergeCommit(commit, remote, options));
...
async function parseMergeCommit(commit, remote, options = {}) {
  ...
  const command = `git log "${hash}^..${hash}"`;
  const format = await getLogFormat();
  console.log(`${command} --shortstat --pretty=format:${format}`);
  const log = await (0, _utils.cmd)(`${command} --shortstat --pretty=format:${format}`);
  console.log(log);
  const commits = log.split(COMMIT_SEPARATOR).slice(1).map(commit => parseCommit(commit, remote, options));
  return commits;
}
...
function getMerge(message, author, remote, options = {}) {
  ...
  for (let pattern of patterns) {
    ...
    if (match) {
      ...
      return {
        id,
        message: (0, _utils.replaceText)(message, options),
