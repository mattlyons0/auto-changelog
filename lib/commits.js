...
async function parseMergeCommit(commit, remote, options = {}) {
  ...
  const format = await getLogFormat();
  const log = commit + (await (0, _utils.cmd)(`${command} --shortstat --pretty=format:${format}`));
  const commits = log.split(COMMIT_SEPARATOR).map(commit => parseCommit(commit, remote, options));
  return commits;
}
...
function parseCommit(commit, remote, options = {}) {
  ...
  return {
    ...
    body: (0, _utils.replaceText)(getBody(message), options).trim(),
